...
export class ExerciseService {
  ...
  async createExercise(
    ...
    const workout = await this.workoutRepository.findOneBy({ id: workoutId });
    if (!workout) {
  ...
  async deleteExerciseById(id: string): Promise<void> {
    const result = await this.exerciseService.delete({ id });
  ...
  async updateExercise(
    ...
    const exercise = await this.exerciseService.findOne({ where: { id } });
    if (!exercise) {
